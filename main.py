...
if price <= good_price:
    sender = EmailSender() # Type your Gmail address and Google Account's app password
    sender.connect()
    sender.compose(target="unitsocial@zohomail.com", from_name="TI-84 Price Tracker", to_name="You", subject="Great Amazon Deals for a TI-84 On Amazon",
                   message=f"The TI-84 Graphing Calculator on Amazon is only ${price}! Buy it while you can!") # Feel free to play around with this
    sender.send()
    sender.close()